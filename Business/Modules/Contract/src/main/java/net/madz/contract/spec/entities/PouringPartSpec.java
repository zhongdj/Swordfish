...
public class PouringPartSpec extends MultiTenancyEntity {
    ...
    @JoinTable(name = "pouring_part_spec_additive", joinColumns = {
            @JoinColumn(table = "pouring_part_spec", name = "TENANT_ID", nullable = false, insertable = false,
                    updatable = false, referencedColumnName = "TENANT_ID"),
            @JoinColumn(table = "pouring_part_spec", name = "POURING_PART_SPEC_ID", updatable = true,
                    insertable = true, referencedColumnName = "ID") },
            inverseJoinColumns = { @JoinColumn(table = "additive", name = "ADDITIVE_ID", updatable = true,
                    insertable = true, referencedColumnName = "ID") })
