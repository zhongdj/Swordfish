    ...
    public boolean auditPassword(String password) throws PasswordException {
        ...
        if ( auditPolicy.isPasswordMinLengthEnabled() ) {
            if ( password.length() < auditPolicy.getPasswordMinLength() ) {
                throw new PasswordException("the password's length is too short, the min length of password is " + auditPolicy.getPasswordMinLength());
            }
        }
        if ( auditPolicy.isPasswordMaxLengthEnabled() ) {
            if ( password.length() > auditPolicy.getPasswordMaxLength() ) {
                throw new PasswordException("the password's length is too long, the max length of password is " + auditPolicy.getPasswordMaxLength());
            }
        }
        ...
        if ( auditPolicy.isSpecialCharactersMustEnabled() ) {
            ...
            if ( !result ) {
                throw new PasswordException("the password does not include any special characters in: " + String.valueOf(auditPolicy.getSpecialCharacters()));
            }
        }
        if ( auditPolicy.isCommonCharactersMustEnabled() ) {
            ...
            if ( !result ) {
                throw new PasswordException("the password does not include any common characters in: " + String.valueOf(auditPolicy.getCommonCharacters()));
            }
        }
